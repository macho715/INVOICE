...
## 🔄 최신 업데이트 (v2.2 - 2025-11-04)

### 월별 파라미터 시스템
- **클래스**: `SHPTEnhancedAuditSystem` (월별 파라미터 지원)
- **사용법**: `SHPTEnhancedAuditSystem(month="Oct", year=2025)`
- **장점**: 코드 중복 제거, 모든 월 지원 가능

### PDF 파서 통합
- **모듈**: `PDF/praser.py`의 `DSVPDFParser` 클래스
- **기능**: 실제 PDF 내용 파싱 (BOE, DO, DN, CarrierInvoice)
- **통합**: `enable_pdf_parsing` 파라미터로 조건부 활성화
- **결과**: 파일명 기반 → 실제 데이터 기반 검증으로 전환

### Gate 검증 확장
- **Gate-08 추가**: PDF 파싱 데이터 일관성 검증
- **검증 항목**: BOE 금액, Container Numbers, HS Code
- **효과**: 평균 Gate Score 78.8 → 80.0 향상

### 아키텍처 개선
```
SHPTEnhancedAuditSystem
├── 월별 파라미터 (month, year)
├── PDF 파서 (DSVPDFParser) - 조건부
├── Gate 검증 (Gate-01, Gate-07, Gate-08)
└── 증빙문서 매핑 (파일명 + PDF 파싱)
```

**상세 내용**: `Documentation/OCT_2025_INTEGRATION_REPORT.md` 참조
