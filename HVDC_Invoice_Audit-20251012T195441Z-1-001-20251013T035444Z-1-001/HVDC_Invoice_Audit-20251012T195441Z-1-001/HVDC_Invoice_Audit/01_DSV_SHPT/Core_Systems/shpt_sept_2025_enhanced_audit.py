#!/usr/bin/env python3
"""
SHPT Enhanced Invoice Audit System (Month-Agnostic)

SHPT 시스템 + Portal Fee 검증 + Gate 검증 + 월별 인보이스 지원
- Excel 직접 처리
- Portal Fee ±0.5% 검증
- 핵심 Gate 검증 (3개)
- S/No 순서 보존
...
# PDF Parser import
# PDF 폴더는 HVDC_Invoice_Audit-20251012T195441Z-1-001/PDF/ 에 있음
...
try:
    ...
    # 로깅은 __init__ 이후에 설정되므로 여기서는 경고만 출력
    pass  # 로깅은 나중에 처리


class SHPTEnhancedAuditSystem:
    """SHPT Enhanced 감사 시스템 (월별 파라미터 지원)"""
    ...
    def __init__(self, month: str = "Sept", year: int = 2025, enable_pdf_parsing: bool = True):
        """
        Args:
            month: 월 약자 (예: "Sept", "Oct")
            year: 연도 (예: 2025)
            enable_pdf_parsing: PDF 파싱 활성화 여부 (기본값: True)
        """
        self.month = month
        self.year = year
        ...
        # 출력 디렉토리: Results/{month}_{year}/
        self.out_dir = self.root / "Results" / f"{self.month}_{self.year}"
        self.out_dir.mkdir(parents=True, exist_ok=True)

        # 월별 파일 경로 생성
        month_num = self.MONTH_NUMBER_MAP.get(self.month, "01")
        month_name_excel = self.MONTH_NAME_MAP.get(self.month, self.month.upper())
        
        # Data 폴더: DSV {year}{month_num}
        data_folder = self.root / "Data" / f"DSV {self.year}{month_num}"
        
        # Excel 파일: SCNT SHIPMENT DRAFT INVOICE ({MONTH_NAME} {YEAR}).xlsm
        self.excel_file = (
            data_folder
            / f"SCNT SHIPMENT DRAFT INVOICE ({month_name_excel} {self.year}).xlsm"
        )
        
        # 증빙문서 경로 (여러 패턴 시도)
        supporting_docs_paths = []
        
        # 패턴 1: SCNT (HVDC) Import Shipments ({Month} {Year}) - Supporting Documents
        path1 = data_folder / f"SCNT (HVDC) Import Shipments ({self.month.capitalize()} {self.year}) - Supporting Documents"
        if path1.exists():
            supporting_docs_paths.append(path1)
        
        # 패턴 2: SCNT Import ({Month} {Year}) - Supporting Documents
        path2 = data_folder / f"SCNT Import ({self.month.capitalize()} {self.year}) - Supporting Documents"
        if path2.exists():
            supporting_docs_paths.append(path2)
        
        # 패턴 3: SCNT Domestic ({Month} {Year}) - Supporting Documents
        path3 = data_folder / f"SCNT Domestic ({self.month.capitalize()} {self.year}) - Supporting Documents"
        if path3.exists():
            supporting_docs_paths.append(path3)
        
        # 패턴이 없으면 기본 경로 추가 (존재 여부와 관계없이)
        if not supporting_docs_paths:
            supporting_docs_paths.append(path1)  # 기본 패턴
        
        self.supporting_docs_paths = supporting_docs_paths
        ...
        if self.enable_pdf_parsing:
            try:
                ...
                logging.warning(f"⚠️ PDF Parser 초기화 실패: {e}. 파일명 기반 매핑으로 fallback.")
                self.enable_pdf_parsing = False
                self.pdf_parser = None
        else:
            logging.info("📄 PDF Parser 비활성화됨 (파일명 기반 매핑 사용)")
        ...
        # 로깅 설정
        log_file = self.out_dir / f"shpt_{self.month.lower()}_{self.year}_enhanced_audit.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        logging.info(f"SHPT Enhanced 시스템 초기화 완료 ({self.month} {self.year})")
    ...
    def validate_gate_08_pdf_data_consistency(
        ...
        """Gate-08: PDF 파싱 데이터와 인보이스 일치 검증 (Enhanced)"""
        score = 100
        issues = []
        validated_fields = []
            ...
            if boe_doc.get("parsed_data"):
                ...
                if boe_value_usd is not None:
                    ...
                    if delta_pct > 5.0:  # 5% 이상 차이
                        issues.append(
                            f"BOE 금액 불일치: 인보이스 ${item_total:.2f}, BOE ${boe_value_usd:.2f} (Δ{delta_pct:.1f}%)"
                        )
                        score -= 20
    ...
    def map_supporting_documents(self) -> Dict[str, List[Dict]]:
        """증빙문서 매핑 생성 (PDF 파싱 포함)"""
        supporting_docs = {}
        parsing_stats = {"success": 0, "failed": 0, "skipped": 0}
                        ...
                        if self.enable_pdf_parsing and self.pdf_parser:
                            try:
                                parsed_result = self.pdf_parser.parse_pdf(
                                    str(pdf_file), doc_type=doc_type
                                )
                                ...
                                if parsed_result.get("error") is None:
                                    ...
                                    if parsed_result.get("data"):
                                        ...
                                        # DO 데이터 추출
                                            ...
                                            doc_info["do_number"] = data.get("do_number")
                                            doc_info["mbl_no"] = data.get("mbl_no")
                                            doc_info["containers"] = [
                                        ...
                                        # DN 데이터 추출
                                            ...
                                            doc_info["waybill_no"] = data.get("waybill_no")
                                            doc_info["container_no"] = data.get("container_no")
                                else:
                                    # 파싱 에러
                                    doc_info["parsing_error"] = parsed_result.get("error")
                                    parsing_stats["failed"] += 1
                                    logging.debug(
                                        f"PDF 파싱 실패 ({pdf_file.name}): {parsed_result.get('error')}"
                                    )

                            except Exception as e:
                                # 파싱 예외 처리
                                doc_info["parsing_error"] = str(e)
                                parsing_stats["failed"] += 1
                                logging.debug(f"PDF 파싱 예외 ({pdf_file.name}): {e}")

                        else:
                            # PDF 파싱 비활성화
                            parsing_stats["skipped"] += 1
        ...
        if self.enable_pdf_parsing:
            logging.info(
                f"[PDF PARSING] Success: {parsing_stats['success']}, "
                f"Failed: {parsing_stats['failed']}, "
                f"Skipped: {parsing_stats['skipped']}"
            )
    ...
    def extract_shipment_id(self, filename: str) -> Optional[str]:
        ...
        # 패턴 1: "01. HVDC-ADOPT-SCT-0136_SupportingDocs.pdf" 형식
        # 패턴 2: "HVDC-ADOPT-SCT-0126_BOE.pdf" 형식
    ...
    def run_full_enhanced_audit(self):
        ...
        try:
            logging.info("=" * 80)
            logging.info(f"[START] SHPT Enhanced {self.month} {self.year} full audit")
            logging.info("=" * 80)
            ...
            for sheet_name in excel_file.sheet_names:
                # 시트 이름 필터링 (Summary, Template, 월 이름 등 제외)
                excluded_names = ["Summary", "Template", "SEPT", "OCTOBER", "NOVEMBER", 
                                 "DECEMBER", "JANUARY", "FEBRUARY", "MARCH", "APRIL", 
                                 "MAY", "JUNE", "JULY", "AUGUST"]
                if sheet_name.startswith("_") or sheet_name in excluded_names:
                    continue
    ...
    def save_enhanced_results(self, audit_result):
        ...
        try:
            ...
            json_file = (
                self.out_dir
                / "JSON"
                / f"shpt_{self.month.lower()}_{self.year}_enhanced_result_{timestamp}.json"
            )
            ...
            # CSV 파일 저장 (새 폴더 구조)
            csv_file = (
                self.out_dir / "CSV" / f"shpt_{self.month.lower()}_{self.year}_enhanced_result_{timestamp}.csv"
            )
            ...
            summary_file = (
                self.out_dir
                / "Reports"
                / f"shpt_{self.month.lower()}_{self.year}_enhanced_summary_{timestamp}.txt"
            )
            summary_file.parent.mkdir(parents=True, exist_ok=True)
            with open(summary_file, "w", encoding="utf-8") as f:
                f.write("=" * 80 + "\n")
                f.write(f"SHPT Enhanced {self.month} {self.year} Invoice Audit 결과\n")
                f.write("=" * 80 + "\n\n")
