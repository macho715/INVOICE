...
def print_banner():
    """파이프라인 시작 배너를 출력합니다."""
    print("\n" + "=" * 80)
    print("HVDC PIPELINE v3.0.1 - Integration Execution")
    ...
    print("   Stage 3: Report Generation + Toolkit 컬럼 정규화")
    print("   Stage 4: PyOD 앙상블 ML + 이상치 시각화")
    print("=" * 80 + "\n")
...
def run_stage(
    ...
    try:
        if stage_num == 1:
            ...
            # Select synchronizer based on no_sorting flag
            ...
            if visualize:
                if AnomalyVisualizer is None:
                    ...
                    try:
                        ...
                        viz_sheet = sheet_name if isinstance(sheet_name, str) else "통합_원본데이터_Fixed"
                        ...
                        if viz_result.get("success"):
                            logger.info(
                                "Stage 4 시각화 표시 완료: %s", viz_result.get("message")
                            )
                            ...
                            logger.error(
                                "Stage 4 시각화 표시 실패: %s", viz_result.get("message")
                            )
