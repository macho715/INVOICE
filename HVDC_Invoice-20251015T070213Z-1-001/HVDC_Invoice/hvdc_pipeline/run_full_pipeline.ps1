...
Write-Host "HVDC 파이프라인 전체 실행 스크립트" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $SyncedFile = "data\processed\synced\HVDC WAREHOUSE_HITACHI(HE).synced_v2.9.4.xlsx"
    ...
    # Stage 2-3: Derived Columns & Report Generation
    Write-Host "Stage 2-3: Derived Columns & Report Generation 실행 중..." -ForegroundColor Yellow
    ...
    Write-Host "Stage 4: Anomaly Detection 실행 중... (이상치 색상 적용 포함)" -ForegroundColor Yellow
    ...
    Write-Host "Stage 4 완료! (이상치 색상 적용됨, 소요시간: $($Stage4Duration.TotalSeconds.ToString('F1'))초)" -ForegroundColor Green
    Write-Host ""
