# HVDC Pipeline v4.0.2

**Samsung C&T Logistics | ADNOC·DSV Partnership**

통합된 HVDC 파이프라인으로 데이터 동기화부터 이상치 탐지까지 전체 프로세스를 자동화합니다.

## 🚀 최근 업데이트 (v4.0.3 - Missing Columns Auto-Generation)

### 중요 기능 추가 (2025-10-22)
- **✨ Stage 1 Auto-Column Generation**: header_registry.py의 모든 location 컬럼 자동 추가
- **🔧 Missing Columns Fix**: JDN MZD, AAA Storage 등 누락된 컬럼 자동 생성
- **📊 Single Source of Truth**: header_registry.py를 유일한 정보원으로 사용
- **🎯 Future-Proof**: 새 창고 추가 시 코드 변경 불필요
...
### 전체 실행 시간 (v4.0.2 - Path Fix)
- Stage 1 (v3.0 Multi-Sheet): ~36초 ⚡ (3 sheets 병합 + DSV WH 통합 + 안정 정렬)
- Stage 2 (파생 컬럼): ~16초
- Stage 3 (보고서 + Path Fix): ~115초 (Stage 2 derived 폴더에서 읽기)
- Stage 4 (이상치 + 색상): ~50초 (탐지 + 색상화)
- **총 실행 시간**: ~217초 (약 3분 37초)
...
**⚠️ 중요**: Stage 4 색상화를 활성화하려면 `--stage4-visualize` 플래그가 필요합니다:
```bash
...
- [Core Module 통합 보고서](CORE_MODULE_INTEGRATION_REPORT.md) - Semantic matching 시스템 상세
- [최종 통합 요약](FINAL_INTEGRATION_SUMMARY.md) - v4.0.1 변경 사항
