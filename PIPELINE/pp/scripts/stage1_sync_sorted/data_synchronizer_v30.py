...
class DataSynchronizerV30:
    ...
    def synchronize(
        ...
        try:
            ...
            master_sheets = self._load_file_by_sheets(master_xlsx, "Master")
            warehouse_sheets = self._load_file_by_sheets(warehouse_xlsx, "Warehouse")
            ...
            common_sheets = set(master_sheets.keys()) & set(warehouse_sheets.keys())
            master_only_sheets = set(master_sheets.keys()) - set(warehouse_sheets.keys())
            warehouse_only_sheets = set(warehouse_sheets.keys()) - set(master_sheets.keys())
            ...
            # Process common sheets
            for sheet_name in common_sheets:
                print(f"\n--- Processing sheet: '{sheet_name}' ---")
                
                m_df, m_header_row = master_sheets[sheet_name]
                w_df, w_header_row = warehouse_sheets[sheet_name]

                # Match headers for this sheet
                master_columns = self._match_and_validate_headers(m_df, f"Master-{sheet_name}")
                warehouse_columns = self._match_and_validate_headers(w_df, f"Warehouse-{sheet_name}")
                ...
                processed_sheets[sheet_name] = (updated_w_df, w_header_row)
                
                # Accumulate stats
                for key in total_stats:
                    total_stats[key] += stats.get(key, 0)

                print(f"  [OK] {sheet_name}: {len(updated_w_df)} rows, {stats.get('updates', 0)} updates")
            ...
            out = output_path or str(
                Path(warehouse_xlsx).with_name(Path(warehouse_xlsx).stem + ".synced_v3.4_multi.xlsx")
            )
            ...
            # Save merged file
            merged_output_path = Path(out).with_name(Path(out).stem.replace("_multi", "_merged") + ".xlsx")
            ...
            print(f"Total changes: {total_stats['updates']} updates, {total_stats['appends']} new records")
