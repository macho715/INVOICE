...
class FileRegistry:
    ...
    def get_master_file(cls, vendor: str = "hitachi") -> str:
        ...
        if path is None:
            raise ValueError(
                f"Unknown vendor: {vendor}. Available: {list(cls.RAW_FILES['master'].keys())}"
            )
        return path
    ...
    def get_warehouse_file(cls, vendor: str = "hitachi") -> str:
        ...
        if path is None:
            raise ValueError(
                f"Unknown warehouse vendor: {vendor}. Available: {list(cls.RAW_FILES['warehouse'].keys())}"
            )
        return path

    @classmethod
    def get_synced_file(cls, version: str | None = None, merged: bool = False) -> str:
        """
    ...
    def get_derived_file(cls, vendor: str = "hitachi") -> str:
        ...
        if filename is None:
            raise ValueError(
                f"Unknown vendor: {vendor}. Available: {list(cls.PROCESSED_FILES['derived'].keys())}"
            )
        return str(Path(cls.DIRECTORIES["processed"]["derived"]) / filename)

    @classmethod
    def get_report_file(cls, timestamp: str | None = None) -> str:
        """
    ...
    def get_anomaly_file(cls, format: str = "excel") -> str:
        ...
        if filename is None:
            raise ValueError(
                f"Unknown format: {format}. Available: {list(cls.PROCESSED_FILES['anomaly'].keys())}"
            )
        return str(Path(cls.DIRECTORIES["anomaly"]) / filename)

    @classmethod
    def get_directory(cls, category: str, subcategory: str | None = None) -> str:
        """
        Get directory path.
    ...
    def get_sheet_variants(cls, sheet_type: str) -> list[str]:
        ...
        if variants is None:
            raise ValueError(
                f"Unknown sheet type: {sheet_type}. Available: {list(cls.SHEET_NAMES.keys())}"
            )
        return variants
    ...
    @classmethod
    def guess_vendor_from_file(cls, file_path: str) -> str | None:
...
def get_synced_file(version: str | None = None, merged: bool = False) -> str:
