...
def normalize_case_series(
    series: pd.Series, profile: NormalizationProfile | None = None
) -> pd.Series:
...
def compute_global_dup_flags(
    ...
    summary = pd.DataFrame(
        summary_rows, columns=["File Name", "Sheet Name", "고유", "중복", "총합계"]
    )
...
def drop_duplicates_by_case(
    ...
    keep: Literal["first", "last", "False"] = "first",
    profile: NormalizationProfile | None = None,
    preserve_empty_keys: bool = True,  # 빈 키 행 보존 옵션
) -> pd.DataFrame:
    """
    Drop duplicates by Case No key, with option to preserve empty keys.
    
    Args:
        df: DataFrame to process
        semantic_finder: Optional function to find Case No column
        keep: Which duplicate to keep ('first', 'last', or 'False' to keep none)
        profile: Normalization profile for key normalization
        preserve_empty_keys: If True, preserve rows with empty/NaN Case No values
    ...
    if preserve_empty_keys:
        # 원본 컬럼 값 기준으로 빈 키 판단 (정규화 전)
        # NaN, 빈 문자열, "nan", "NaN", "None" 등을 모두 빈 키로 처리
        original_col_str = df[col].astype(str).str.strip()
        original_col_empty = (
            df[col].isna() 
            | (original_col_str == "") 
            | (original_col_str.isin(["nan", "NaN", "None", "N/A", "NA", ""]))
        )
        
        # 정규화된 키도 빈 문자열인지 확인
        # 정규화 과정에서 빈 문자열로 변환된 것도 빈 키로 처리
        normalized_empty = (key.isna()) | (key == "")
        
        # 두 조건 모두 만족하는 것이 진짜 빈 키
        # 빈 키는 중복 제거 대상에서 제외하고 모두 보존
        empty_mask = original_col_empty | normalized_empty
        
        empty_rows = df[empty_mask].copy()
        non_empty_rows = df[~empty_mask].copy()
        
        # 비어있지 않은 키에 대해서만 중복 제거
        if len(non_empty_rows) > 0:
            non_empty_key = normalize_case_series(non_empty_rows[col], profile)  # type: ignore[index]
            non_empty_mask = ~non_empty_key.duplicated(keep=(None if keep == "False" else keep))
            deduped_non_empty = non_empty_rows.loc[non_empty_mask].copy()
        else:
            deduped_non_empty = non_empty_rows
        
        # 빈 키 행과 중복 제거된 행 결합 (빈 키 행은 모두 보존)
        if len(empty_rows) > 0 and len(deduped_non_empty) > 0:
            result = pd.concat([deduped_non_empty, empty_rows], ignore_index=True, sort=False)
            return result
        elif len(empty_rows) > 0:
            return empty_rows.copy()
        else:
            return deduped_non_empty
    else:
        # 기존 동작: 모든 행에 대해 중복 제거
...
def drop_duplicates_global(
    ...
    for idx, df in enumerate(dfs, start=1):
        tdf, dup = mark_duplicates(
            df, semantic_finder=semantic_finder, keep="first", profile=profile
        )
        diag_rows.append(
            {
                "part": f"sheet_{idx}",
    ...
    if not merged.empty:
        merged = merged.drop_duplicates(
            subset=[_CASE_KEY], keep=(None if keep == "False" else keep)
        )
...
def validate_duplicate_removal(diag_df: pd.DataFrame) -> Dict[str, float]:
    total_rows = (
        float(diag_df.get("rows", pd.Series(dtype=float)).sum()) if not diag_df.empty else 0.0
    )
    total_dups = (
        float(diag_df.get("dups", pd.Series(dtype=float)).sum()) if not diag_df.empty else 0.0
    )
