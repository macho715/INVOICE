...
Usage:
    ...
    # Legacy class method usage (backward compatible)
    master_file = FileRegistry.get_master_file('hitachi')
    warehouse_file = FileRegistry.get_warehouse_file('hitachi')
    
    # New instance-based usage with auto-detection
    registry = FileRegistry(Path(__file__).resolve().parents[1])
    he_path, he_name = registry.get_raw_with_canonical("HE")
...
# Import file_finder for auto-detection
try:
    from .file_finder import find_vendor_files, RawFileHit
except ImportError:
    try:
        from scripts.core.file_finder import find_vendor_files, RawFileHit
    except ImportError:
        find_vendor_files = None
        RawFileHit = None
...
class FileRegistry:
    ...
    # ==================== INSTANCE-BASED AUTO-DETECTION (NEW) ====================
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize FileRegistry instance with auto-detection capabilities.
        
        Args:
            project_root: Project root directory (defaults to current working directory)
            enable_siemens_alias: Include 'siemens' as SIM token (default True)
            extra_sim_tokens: Additional tokens for SIM vendor matching
            extra_he_tokens: Additional tokens for HE vendor matching
            allowed_suffixes: Override Excel file suffixes set
        """
        if project_root is None:
            ...
            if (cwd / "scripts" / "core" / "file_registry.py").exists():
                project_root = cwd
            elif (cwd.parent / "scripts" / "core" / "file_registry.py").exists():
    ...
    def _ensure_auto_scan(self) -> None:
        """Ensure raw files are scanned and cached."""
        if self._auto_raw_cache is None:
            if find_vendor_files is None:
                self._auto_raw_cache = {"SIM": None, "HE": None}
            else:
