...
class DataSynchronizerV30:
    ...
    def _validate_excel_file(self, file_path: str, file_label: str) -> None:
        ...
        Raises:
            ...
            ValueError: If file has invalid extension, is empty, or is corrupted
            PermissionError: If file is locked by another program
        """
        from pathlib import Path
        import zipfile
        ...
        if path.suffix.lower() not in ['.xlsx', '.xlsm', '.xls']:
            raise ValueError(
                f"{file_label} file has invalid extension: {path.suffix}. "
                f"Expected .xlsx, .xlsm, or .xls"
            )
        ...
        # 4. ZIP 형식 확인 (xlsx/xlsm는 ZIP 기반)
        if path.suffix.lower() in ['.xlsx', '.xlsm']:
            try:
                # PyInstaller frozen 환경에서 안정성을 위해 Path 객체를 문자열로 변환
                with zipfile.ZipFile(str(path), 'r') as zf:
                    # 파일이 유효한 ZIP인지 확인
                    zf.testzip()
            except zipfile.BadZipFile:
                raise ValueError(
                    f"{file_label} file is not a valid Excel file (ZIP format error): {file_path}. "
                    f"File may be corrupted or not an Excel file. "
                    f"File size: {file_size:,} bytes"
                )
            except PermissionError:
                raise PermissionError(
                    f"{file_label} file is locked by another program: {file_path}. "
    ...
    def _load_file_with_header_detection(
        ...
        try:
            ...
            self._validate_excel_file(file_path, file_label)
            xl = pd.ExcelFile(file_path, engine="openpyxl")
        except (zipfile.BadZipFile, ValueError) as e:
            if "ZIP format" in str(e) or "zip file" in str(e).lower() or "not a valid Excel file" in str(e):
                raise ValueError(
                    f"Failed to open {file_label} file: {file_path}. "
                    f"The file appears to be corrupted or not a valid Excel file. "
                    f"Error: {str(e)}"
                )
            raise
        except PermissionError as e:
            raise PermissionError(
                f"Cannot read {file_label} file: {file_path}. "
                f"The file may be open in another program. "
                f"Please close it and try again. Error: {str(e)}"
            )
        except FileNotFoundError:
            raise  # Already handled by _validate_excel_file
        except Exception as e:
            raise RuntimeError(
                f"Unexpected error loading {file_label} file: {file_path}. "
