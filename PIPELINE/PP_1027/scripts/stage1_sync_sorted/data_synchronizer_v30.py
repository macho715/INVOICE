...
class DataSynchronizerV30:
            ...
            for sheet_name, siemens_df in siemens_sheets.items():
                if sheet_name in master_sheets:
                    ...
                    if case_col and case_col in merged_df.columns:
                        ...
                        if removed > 0:
                            print(
                                f"[DEDUP] Removed {removed} duplicate Case No entries from merged data (column: {case_col})"
                            )
    ...
    def _apply_updates(
        ...
        stats = dict(updates=0, date_updates=0, field_updates=0, appends=0, fallback_matches=0, fallback_vendors={}, vendor_updates=0)

        # Build warehouse index by case number
        wh_case_col = wh_cols["case_number"]
        wh_index = self._build_case_index(wh, wh_case_col)

        # ✅ Build fallback index for SCT cargo (Vendor="SCT")
        # Use SCT Ref.No or Description as fallback matching key
        wh_fallback_index = {}
        sct_ref_col = wh_cols.get("sct_ref_no") if "sct_ref_no" in wh_cols else None
        if not sct_ref_col:
            # Try to find SCT Ref.No column directly
            for col in wh.columns:
                if "sct" in str(col).lower() and "ref" in str(col).lower():
                    sct_ref_col = col
                    break
        
        if sct_ref_col and sct_ref_col in wh.columns:
            # Build index by SCT Ref.No (normalized)
            sct_series = wh[sct_ref_col].fillna("").astype(str).str.strip().str.upper()
            for i, val in enumerate(sct_series.tolist()):
                if val and val not in wh_fallback_index:
                    wh_fallback_index[val] = i
        ...
        for mi, mrow in master.iterrows():
            ...
            # Initialize matched_wi for fallback matching
            matched_wi = None

            # Check if case exists in warehouse
            if key not in wh_index:
                # ✅ FALLBACK: Try to match using Description or other identifiers for any Vendor
                # This handles cases where Case No format differs between Master and Warehouse
                # (e.g., SCT: "DCS NETWORK-04" vs Warehouse: "191221", PPL: "HVDC-DAS 1/3", etc.)
                master_vendor = str(mrow.get("Vendor", "")).strip().upper() if pd.notna(mrow.get("Vendor")) else ""
                # Apply fallback matching for any Vendor if Case No match failed
                if "Vendor" in master.columns and master_vendor:
                    # Try matching by SCT Ref.No if available
                    if sct_ref_col and sct_ref_col in wh.columns:
                        # Check if Master has any identifier that matches Warehouse SCT Ref.No
                        # For SCT cargo, Description might contain SCT Ref.No pattern
                        master_desc = str(mrow.get("Description", "")).strip().upper() if pd.notna(mrow.get("Description")) else ""
                        master_case_raw = str(mrow.get(master_case_col, "")).strip().upper() if pd.notna(mrow.get(master_case_col)) else ""
                        
                        # Try to find matching warehouse row by Description or other identifiers
                        # Strategy: Look for key words in Description that might match
                        master_desc_keywords = set()
                        if master_desc:
                            # Extract meaningful keywords (length > 3, not common words)
                            common_words = {"CABINET", "PACKAGE", "OF", "THE", "AND", "OR"}
                            keywords = [w.strip() for w in master_desc.split() if len(w.strip()) > 3 and w.strip().upper() not in common_words]
                            master_desc_keywords = set(k.upper() for k in keywords)
                        
                        # Also check if Master Case No contains pattern that might match Warehouse SCT Ref.No
                        # e.g., "DCS NETWORK-04" might relate to specific SCT Ref.No patterns
                        
                        for wi, wh_row in wh.iterrows():
                            wh_sct_ref = str(wh_row.get(sct_ref_col, "")).strip().upper() if pd.notna(wh_row.get(sct_ref_col)) else ""
                            wh_desc = str(wh_row.get("Description", "")).strip().upper() if pd.notna(wh_row.get("Description")) else ""
                            wh_case = str(wh_row.get(wh_case_col, "")).strip().upper() if pd.notna(wh_row.get(wh_case_col)) else ""
                            ...
                            if master_desc_keywords and len(master_desc_keywords) >= 2:
                                ...
                                if len(overlap) >= 2:  # At least 2 keywords must match
                                    ...
                                    print(f"      [FALLBACK] Vendor={master_vendor}: Matched Master Case '{master_case_raw}' (Desc: '{master_desc[:50]}...') to Warehouse row {wi} via Description keywords ({len(overlap)} matches)")
                                    break
                            
                            # Match strategy 2: Master Case No pattern in SCT Ref.No
                            if master_case_raw and wh_sct_ref:
                                # Check if any significant part of master case appears in SCT Ref.No
                                master_parts = [p for p in master_case_raw.split("-") if len(p) > 2]
                                if master_parts and any(part in wh_sct_ref for part in master_parts):
                                    matched_wi = wi
                                    matched_wh_rows.add(wi)
                                    print(f"      [FALLBACK] Vendor={master_vendor}: Matched Master Case '{master_case_raw}' to Warehouse SCT Ref.No '{wh_sct_ref}' via pattern")
                                    break
                            
                            # Match strategy 3: Description substring match (case-insensitive, improved: longer substring)
                            if master_desc and wh_desc and len(master_desc) > 15:
                                # Check if substantial part of master description is in warehouse description
                                for i in range(len(master_desc) - 14):
                                    substring = master_desc[i:i+15]  # Increased from 10 to 15
                                    if substring in wh_desc:
                                        matched_wi = wi
                                        matched_wh_rows.add(wi)
                                        print(f"      [FALLBACK] Vendor={master_vendor}: Matched Master Case '{master_case_raw}' to Warehouse row {wi} via Description substring")
                                        break
                                if matched_wi is not None:
                                    break
                
                if matched_wi is not None:
                    # Found match via fallback - update existing warehouse row
                    wi = matched_wi
                    # Continue to update logic below (skip the append logic)
                    stats["fallback_matches"] = stats.get("fallback_matches", 0) + 1
                    stats["fallback_vendors"] = stats.get("fallback_vendors", {})
                    stats["fallback_vendors"][master_vendor] = stats["fallback_vendors"].get(master_vendor, 0) + 1
                    
                    # ✅ Ensure Vendor column exists in Warehouse
                    if "Vendor" not in wh.columns:
                        wh["Vendor"] = None  # Initialize for all rows
                    
                    # ✅ Ensure Vendor column is updated for fallback matches
                    if "Vendor" in master.columns and wi < len(wh):
                        mval = mrow["Vendor"]
                        old_val = wh.at[wi, "Vendor"] if "Vendor" in wh.columns else None
                        wh.at[wi, "Vendor"] = mval
                        if old_val != mval:
                            stats["vendor_updates"] = stats.get("vendor_updates", 0) + 1
                else:
            ...
            # Case exists - update it (either via Case No match or SCT fallback match)
            if matched_wi is None:
                wi = wh_index[key]
            ...
            # ✅ Process Vendor column as master-only (for all vendors: SCT, PPL, MOSB, etc.)
            if "Vendor" in master.columns:
                mval = mrow["Vendor"]
                
                # Ensure Warehouse has Vendor column
                if "Vendor" not in wh.columns:
                    wh["Vendor"] = None  # Initialize with None for all rows
                
                # Update Vendor if Master has value (for both matched and fallback-matched cases)
                if pd.notna(mval):
                    old_val = wh.at[wi, "Vendor"] if wi < len(wh) else None
                    if old_val != mval:
                        stats["updates"] += 1
                        stats["field_updates"] += 1
                        stats["vendor_updates"] = stats.get("vendor_updates", 0) + 1
                        ...
                        self.change_tracker.add_change(
                            row_index=wi,
                            column_name="Vendor",
                            semantic_key="vendor",  # Use semantic key from header_registry
                            case_no=key,
                            old_value=old_val,
                            new_value=mval,
                            change_type="vendor_update",
                        )
        ...
        if stats.get("fallback_matches", 0) > 0:
            print(f"    - Fallback matches: {stats['fallback_matches']}")
            if stats.get("fallback_vendors"):
                vendor_breakdown = ", ".join([f"{v}={c}" for v, c in stats["fallback_vendors"].items()])
                print(f"      Fallback by vendor: {vendor_breakdown}")
        if stats.get("vendor_updates", 0) > 0:
            print(f"    - Vendor updates: {stats['vendor_updates']}")
        if "source_sheet_updates" in stats:
            print(f"    - Source_Sheet updates: {stats['source_sheet_updates']}")
        
        # ✅ DEBUG: Verify Vendor column values before returning
        if "Vendor" in wh.columns:
            vendor_counts = wh["Vendor"].value_counts(dropna=False)
            non_null_count = wh["Vendor"].notna().sum()
            print(f"  [DEBUG] Vendor in wh DataFrame: {non_null_count}/{len(wh)} non-null values")
            if len(vendor_counts) > 0:
                top_5 = vendor_counts.head(5).to_dict()
                print(f"  [DEBUG] Vendor value counts (top 5): {top_5}")
        ...
        try:
            ...
            with pd.ExcelWriter(out, engine="openpyxl") as writer:
                for sheet_name, (df, header_row) in processed_sheets.items():
                    ...
                    # ✅ keep_unlisted=True to preserve Vendor and other custom columns
                    df_reordered = reorder_dataframe_columns(
                        df, is_stage2=False, keep_unlisted=True, use_semantic_matching=True
                    )
            ...
            merged_df_reordered = reorder_dataframe_columns(
                merged_df, is_stage2=False, keep_unlisted=True, use_semantic_matching=True
            )
