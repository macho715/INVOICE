...
class DataSynchronizerV30:
    ...
    def _apply_master_order_sorting(
        ...
        """
        Sort both DataFrames according to Master's order (v2.9 방식).

        MASTER NO. 순서에 따라 Master와 Warehouse 데이터를 정렬합니다.
        v2.9의 검증된 정렬 로직을 사용하여 안정적인 순서를 보장합니다.
        ...
        # 1. Master를 NO. 순서로 정렬 (v2.9 방식)
        item_col = master_cols.get("item_number")
        if item_col and item_col in master.columns:
            print(f"  Sorting Master by '{item_col}'...")
            # Source_Sheet 정보를 보존하면서 정렬
            master = master.sort_values(item_col, na_position="last").reset_index(drop=True)
            print(f"  Master sorting complete: {len(master)} rows")
        else:
            print(f"  Master already in correct order: {len(master)} rows")

        # 2. Master의 Case No. 순서를 기준으로 Warehouse 정렬
        master_case_order = master[master_case_col].dropna().tolist()
        print(f"  Master Case No. order: {len(master_case_order)} cases")

        # 3. Warehouse에서 Master에 있는 Case No.들만 추출
        wh_master_cases = warehouse[warehouse[wh_case_col].isin(master_case_order)].copy()
        wh_other_cases = warehouse[~warehouse[wh_case_col].isin(master_case_order)].copy()

        print(f"  Warehouse Master cases: {len(wh_master_cases)}")
        print(f"  Warehouse other cases: {len(wh_other_cases)}")

        # 4. Master Case No. 순서에 따라 warehouse_master_cases 정렬
        # v2.9 방식: Master의 정렬된 순서를 그대로 사용
        ...
        # NaN 값 처리: Master에 없는 Case는 맨 뒤로
        wh_master_cases["_sort_order"] = wh_master_cases["_sort_order"].fillna(999999)

        # 정렬 실행
        wh_master_cases = wh_master_cases.sort_values("_sort_order").drop("_sort_order", axis=1)

        # 5. 정렬된 결과: Master 순서 + Master에 없는 Warehouse Case들
        sorted_warehouse = pd.concat([wh_master_cases, wh_other_cases], ignore_index=True)

        # 6. 최종 검증: Warehouse가 Master 순서를 따르는지 확인
        sorted_cases = sorted_warehouse[wh_case_col].dropna().tolist()
        master_in_sorted = sorted_cases[: len(master_case_order)]
        order_match = master_case_order == master_in_sorted
        print(f"  Master cases in correct order: {order_match}")

        if not order_match:
            print(f"    Master order: {master_case_order[:5]}...")
            print(f"    Warehouse order: {master_in_sorted[:5]}...")

        print(f"  Warehouse sorting complete: {len(sorted_warehouse)} rows")
    ...
    def _maintain_master_order(
        ...
        """
        _apply_updates 후에도 MASTER NO. 순서를 유지합니다 (v2.9 방식).
        ...
        print(f"  Master order maintained: {len(sorted_warehouse)} rows")
        print(f"  First 5 Warehouse Case No.: {sorted_warehouse[wh_case_col].head().tolist()}")
