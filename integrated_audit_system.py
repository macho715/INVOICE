"""
증빙문서 통합 Invoice Audit System

AUDIT LOGIC.MD + 실제 증빙문서를 통합한 완전한 송장 감사 시스템입니다.
- 증빙문서 자동 처리 (BOE/DO/DN/Carrier/Port/Airport)
- S/No 순서 보존
- Evidence 링크 실제 PDF 연결
...
class IntegratedAuditSystem:
    ...
    def run_integrated_audit(self) -> Dict:
        """증빙문서 통합 감사 실행"""
        print("🚀 증빙문서 통합 Invoice Audit System 시작")
        print("=" * 70)
        ...
        for sheet_name, df in excel_data.items():
            ...
            invoice_items = self.extract_invoice_data_with_evidence(df, sheet_name, supporting_docs)
        ...
        audit_result = {
            "audit_metadata": {
                ...
                "system_version": "Integrated v4.0",
                "compliance": "FULL_AUDIT_LOGIC_MD_WITH_EVIDENCE"
            },
    ...
    def map_supporting_documents(self) -> Dict[str, List[Dict]]:
        ...
        try:
            # 모든 PDF 파일 스캔
            pdf_files = list(self.supporting_docs_path.rglob("*.pdf"))
            print(f"✅ {len(pdf_files)}개 PDF 파일 발견")
            
            for pdf_file in pdf_files:
                # 파일명에서 Shipment ID와 문서 타입 추출
                shipment_id, doc_type = self.extract_shipment_and_doc_type(pdf_file.name)
    ...
    def extract_shipment_and_doc_type(self, filename: str) -> Tuple[Optional[str], Optional[str]]:
        """파일명에서 Shipment ID와 문서 타입 추출"""
        try:
            # 파일명에서 Shipment ID 추출 (예: HVDC-ADOPT-SCT-0107)
            if "HVDC-ADOPT-" in filename:
                parts = filename.split("_")
                if len(parts) >= 2:
                    shipment_id = parts[0]  # HVDC-ADOPT-SCT-0107
                    doc_part = parts[1].replace(".pdf", "")  # BOE, CarrierInvoice 등
    ...
    def extract_invoice_data_with_evidence(self, df: pd.DataFrame, sheet_name: str, 
        ...
        try:
            ...
            shipment_id = self.extract_shipment_id_from_sheet_name(sheet_name)
            ...
            for idx in range(data_start_row, min(data_end_row + 1, len(df))):
                ...
                if self.is_valid_invoice_row_advanced(row):
                    item = self.create_integrated_audit_item(
    ...
    def extract_shipment_id_from_sheet_name(self, sheet_name: str) -> str:
        """시트명에서 Shipment ID 추출"""
    ...
    def create_integrated_audit_item(self, row: pd.Series, sheet_name: str, 
        ...
        try:
            ...
            # 증빙문서 연결
            item["supporting_documents"] = self.find_related_documents(item, shipment_docs)
            item["evidence_links"] = self.generate_evidence_links(item["supporting_documents"])
    ...
    def find_related_documents(self, item: Dict, shipment_docs: List[Dict]) -> List[Dict]:
        """관련 증빙문서 찾기"""
        related_docs = []
    ...
    def generate_evidence_links(self, supporting_docs: List[Dict]) -> List[str]:
        """Evidence 링크 생성"""
        evidence_links = []
    ...
    def save_audit_report(self, audit_result: Dict) -> str:
        ...
        try:
            # JSON 보고서 저장
            json_file = self.output_dir / "integrated_audit_report.json"
            with open(json_file, 'w', encoding='utf-8') as f:
                json.dump(audit_result, f, indent=2, ensure_ascii=False, default=str)
            
            # CSV 보고서 저장
            csv_file = self.output_dir / "integrated_audit_report.csv"
            df = pd.DataFrame(audit_result["invoice_items"])
            df.to_csv(csv_file, index=False, encoding='utf-8-sig')
            
            # 상세 요약 보고서 저장
            summary_file = self.output_dir / "integrated_audit_summary.txt"
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(self.generate_integrated_summary_report(audit_result))
            
            print(f"📄 증빙문서 통합 감사 보고서 저장 완료:")
    ...
    def generate_integrated_summary_report(self, audit_result: Dict) -> str:
        """증빙문서 통합 요약 보고서 생성"""
        ...
        report = f"""
증빙문서 통합 Invoice Audit System - 상세 보고서
===============================================
...
def main():
    """메인 실행 함수"""
    print("🚀 증빙문서 통합 Invoice Audit System 실행")
    print("=" * 70)
    ...
    # 통합 감사 실행
    audit_result = audit_system.run_integrated_audit()
